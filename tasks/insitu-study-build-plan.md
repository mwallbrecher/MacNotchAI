...
| **C5** daily trace rotation | `TraceRecorder` rotates at the calendar-day boundary, checked per event so an idle machine spends no wake-up asking. This intermediate checkpoint used `v: 2`; new traces now use v5 (with legacy trace-v4 read compatibility) while the affordance schema remains v4. |
